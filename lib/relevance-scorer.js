...
// Initialize OpenAI client lazily to handle missing API key gracefully
let openai = null
function getOpenAI() {
...
async function calculateRelevanceScore(title, summary, whyItMatters, talkTrack, vertical, sourceName) {
  ...
  if (!client) {
    console.log('⚠️ No OpenAI API key configured - using default scoring algorithm')
    const defaultScore = calculateDefaultScore(title, summary, vertical, sourceName)
    console.log(`📊 Default relevance score: ${defaultScore}/100 for "${title.substring(0, 40)}..."`)
    return defaultScore
  }
...
`
    ...
    console.error('⚠️ Error calculating relevance score:', error.message)
    console.error('⚠️ Falling back to default scoring algorithm')
    const defaultScore = calculateDefaultScore(title, summary, vertical, sourceName)
    console.log(`📊 Default relevance score: ${defaultScore}/100`)
    return defaultScore
  }
}
