...
async function generateMetricsAIContent(title, value, source, summary, vertical) {
  ...
  try {
    ...
    const formattedValue = formatValueWithUnit(value, title)
...
Create THREE distinct pieces of sales intelligence:

1. CONTEXT (3-4 sentences) - What this metric means:
   - Define what this metric measures and represents
   - Explain the methodology or calculation behind it
   - Provide context about what ${formattedValue} indicates in the market
   - Focus on EXPLAINING the metric itself, not business implications
   - Use proper unit formatting (e.g., $230.4, 23.5%, $1.2B, etc.)

2. WHY IT MATTERS (3-4 sentences) - How to use this information:
   - Explain the SPECIFIC BUSINESS IMPACT of this exact metric value
   - Describe what this number reveals about market conditions, budget allocations, or competitive landscape
...
REQUIREMENTS:
- Reference the exact metric value with proper formatting in ALL responses
...
- Format values properly: $230.4, 23.5%, $1.2B, etc.
...
`
  ...
  return {
    context: result.context || result.whyItMatters,
    whyItMatters: result.whyItMatters,
    talkTrack: result.talkTrack
  }
}
...
/**
 * Format metric value with appropriate unit symbols
 */
function formatValueWithUnit(value, title) {
  const valueStr = value.toString()
  const titleLower = title.toLowerCase()
  
  // Handle percentages
  if (valueStr.includes('%') || titleLower.includes('percent') || titleLower.includes('rate') || titleLower.includes('growth')) {
    return valueStr.includes('%') ? valueStr : `${valueStr}%`
  }
  
  // Handle currency
  if (valueStr.includes('$') || titleLower.includes('cost') || titleLower.includes('spend') || titleLower.includes('revenue') || titleLower.includes('market size')) {
    if (valueStr.includes('$')) {
      return valueStr
    }
    
    // Check for billions/millions
  ...
  // Handle billions
  if (valueStr.includes('B') || titleLower.includes('billion')) {
    return valueStr.includes('B') ? valueStr : `${valueStr}B`
  }
  
  // Handle millions
  if (valueStr.includes('M') || titleLower.includes('million')) {
    return valueStr.includes('M') ? valueStr : `${valueStr}M`
  }
...
function isMetricContentSpecific(content, value, title, vertical) {
  const { context, whyItMatters, talkTrack } = content
  const fullContent = `${context || ''} ${whyItMatters} ${talkTrack}`.toLowerCase()
