...
async function getNewsletterContent(baseUrl) {
  ...
  try {
    ...
    // FRESHNESS CHECK: Verify metric is from today
    const metric = metricsData.metrics?.[0]
    if (metric) {
      const metricDate = new Date(metric.publishedAt)
      const today = new Date()
      const isToday = metricDate.toDateString() === today.toDateString()
      
      if (!isToday) {
        console.error('❌ STALE METRIC DETECTED:', metric.title, 'from', metricDate.toISOString())
        console.error('❌ NEWSLETTER SEND ABORTED - Content is not fresh')
        throw new Error('Metric is not from today - content refresh may have failed')
      }
      
      console.log('✅ Metric freshness verified - published today:', metricDate.toISOString())
    }
    ...
    // FRESHNESS CHECK: Verify articles are from today
    ...
    if (articles.length > 0) {
      ...
      if (todayArticles.length === 0) {
        console.error('❌ STALE ARTICLES DETECTED - No articles from today')
        console.error('❌ NEWSLETTER SEND ABORTED - Content is not fresh')
        throw new Error('No articles from today - content refresh may have failed')
      }
      
      console.log(`✅ Article freshness verified - ${todayArticles.length} articles from today`)
    }
