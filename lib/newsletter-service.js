...
async function getNewsletterContent(baseUrl) {
  ...
  try {
    ...
    // Sort articles by AI importance score (highest first) and take top 3
...
async function generateSubjectLine(metric, articles) {
  if (!openai) {
    ...
    if (metric) {
      // Extract metric title and include value
      const cleanTitle = metric.title.replace(/\s*\([^)]*\)\s*$/, '').trim()
      const value = metric.value + (metric.unit === 'percentage' || metric.unit === '%' ? '%' : (metric.unit || ''))
      return `🔔 The Beacon: ${cleanTitle} at ${value}`
    }
    if (articles.length > 0) {
      return `🔔 The Beacon: ${articles[0].title.substring(0, 40)}...`
    }
...
2. INCLUDES THE SPECIFIC METRIC VALUE when focusing on metrics (e.g., "Mobile Commerce at 45.2%" not just "Mobile Commerce")
3. Highlights the most actionable insight for sales professionals
4. Creates urgency and curiosity for MarTech/AdTech decision-makers
5. Is under 65 characters total
6. Uses concrete numbers and percentages when available

Examples of good subject lines:
- "🔔 The Beacon: Mobile Commerce at 45.2% - Key Trend Alert"
- "🔔 The Beacon: CTV Ad Spend Hits $18.9B - Talk Track Inside"
- "🔔 The Beacon: 73% of CMOs Shift Budget to AI Tools"
