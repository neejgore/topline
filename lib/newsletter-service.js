...
/**
 * Generate star rating HTML for newsletter
 * Always shows 3 stars, colored based on score
 */
function getStarRatingHTML(importanceScore) {
  const score = importanceScore || 0;
  
  let starCount = 0;
  let starColor = '#d1d5db'; // Gray for uncolored
  
  if (score >= 85) {
    starCount = 3;
    starColor = '#ef4444'; // Red - High relevance
  } else if (score >= 70) {
    starCount = 2;
    starColor = '#f97316'; // Orange - Medium relevance
  } else if (score >= 50) {
    starCount = 1;
    starColor = '#9ca3af'; // Gray - Low relevance
  }
  // score < 50 = 0 stars colored
  
  const unfilledColor = '#e5e7eb'; // Light gray for unfilled stars
  
  return [0, 1, 2].map(i => 
    `<span style="color: ${i < starCount ? starColor : unfilledColor}; font-size: 12px; margin-right: -2px;">★</span>`
  ).join('');
}
        ...
        <div class="section">
            ...
            <div class="article" style="border-left: 4px solid ${articleColors[index % articleColors.length]};">
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                    <span style="display: inline-flex; align-items: center;">${getStarRatingHTML(article.importanceScore)}</span>
                    <span style="background: #e5e7eb; padding: 3px 10px; border-radius: 12px; font-size: 11px; color: #4b5563;">${article.vertical || 'General'}</span>
                </div>
                <h3 style="margin: 0 0 8px 0; font-size: 15px;"><a href="https://belldesk.ai/" style="color: #111827; text-decoration: none;">${decodeHtmlEntities(article.title || '')}</a></h3>
                <p style="margin: 0 0 8px 0; font-size: 14px; color: #6b7280;">${decodeHtmlEntities(article.summary || '')}</p>
                <div class="article-meta" style="font-size: 11px; color: #9ca3af;">
...
async function generateSubjectLine(metric, articles) {
  if (!openai) {
    ...
    return `🔔 Sales Intelligence Brief - ${currentDate}`
  }
...
3. INCLUDES THE SPECIFIC METRIC VALUE when focusing on metrics (e.g., "🔔 Mobile Commerce Hits 45.2%" not just "Mobile Commerce")
4. Highlights the most actionable insight for sales professionals
5. Is clean, clear, and compelling - NOT cheesy or pushy
6. Is under 60 characters total
7. Uses concrete numbers and percentages when available

IMPORTANT RULES:
- Do NOT use cheesy phrases like "Act Now!", "Don't Miss Out!", "Time to Act!", "Key Insights", "Must Read", "Breaking"
- Do NOT include "The Beacon:" or any brand name
- Keep it professional and informative, like a Bloomberg or WSJ headline
- Let the data speak for itself

Examples of good subject lines:
- "🔔 Mobile Commerce Reaches 45.2% of E-commerce"
- "🔔 CTV Ad Spend Hits $18.9B in Q4"
- "🔔 73% of CMOs Now Prioritizing AI Tools"
- "🔔 Retail Media Networks Growing 3x Faster Than Display"
...
`
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert email marketer specializing in B2B sales intelligence newsletters. Write clean, professional subject lines like Bloomberg or WSJ headlines. NEVER use cheesy phrases like 'Act Now', 'Don't Miss Out', 'Key Insights'. Start with 🔔 emoji and let the data speak for itself."
        },
        {
          role: "user",
          content: prompt
